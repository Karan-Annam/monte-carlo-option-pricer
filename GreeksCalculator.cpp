...
double greeks::calculate_delta(Option& option, MonteCarloOptionPricer& pricer, double calc_price){
    ...
    for(int i = 0; i < num_bumps; i++){
        inputs.spot_price = inputs.spot_price + i * bump_size;
        double up_price = pricer.price_option(option, inputs, 100, 1000000);
        inputs.spot_price = temp;

        inputs.spot_price = inputs.spot_price - i * bump_size;
        double down_price = pricer.price_option(option, inputs, 100, 1000000);
        sum_finite_diff += (up_price - down_price) / (2 * bump_size);
        inputs.spot_price = temp;
